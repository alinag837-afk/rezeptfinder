...
// =====================================================
// VERSION 1.61
// Kategorien funktionieren jetzt exakt wie Tags
// =====================================================

window.rf161AktiveKategorie = "";

function rf161KategorieAktualisieren() {

  const buttons = Array.from(
    document.querySelectorAll("#suchKategorieKacheln .kategorie-kachel")
  );
  ...
  buttons.forEach(button => {
    ...
    button.onclick = function (event) {
      ...
      const aktuelleKategorie = button.dataset.kategorie || "";

      // erneut klicken = zurück auf Alle
      if (window.rf161AktiveKategorie === aktuelleKategorie) {
        window.rf161AktiveKategorie = "";
      } else {
        window.rf161AktiveKategorie = aktuelleKategorie;
      }

      // Optik wie bei Tags
      buttons.forEach(b => b.classList.remove("aktiv"));

      const aktiverButton = buttons.find(
        b => (b.dataset.kategorie || "") === window.rf161AktiveKategorie
      );

      if (aktiverButton) {
        aktiverButton.classList.add("aktiv");
      } else {
        const alleButton = buttons.find(
          b => (b.dataset.kategorie || "") === ""
        );
...
// Exakter Kategorienfilter
window.rf161KategoriePasst = function (rezept) {

  if (!window.rf161AktiveKategorie) {
    return true;
  }

  const rezeptKategorie = String(
    rezept.kategorie || "Nicht zugeordnet"
  ).trim().toLowerCase();

  const aktiveKategorie = String(
    window.rf161AktiveKategorie
  ).trim().toLowerCase();

  return rezeptKategorie === aktiveKategorie;
};

// Bestehende Suche erweitern
const rf161OriginalSuche = rezeptSucheAusfuehren;
...
window.rezeptSucheAusfuehren = function () {
  ...
  try {
    ergebnisse = typeof rf161OriginalSuche === "function"
      ? rf161OriginalSuche()
      : rezepte;
  } catch (e) {
    ergebnisse = rezepte;
  ...
  // Falls alte Funktion direkt rendert:
  if (!Array.isArray(ergebnisse)) {
    ergebnisse = rezepte;
  }

  ergebnisse = ergebnisse.filter(r =>
    window.rf161KategoriePasst(r)
  );
...
window.addEventListener("load", function () {
  rf161KategorieAktualisieren();

  setTimeout(rf161KategorieAktualisieren, 300);
  setTimeout(rf161KategorieAktualisieren, 1000);
  setTimeout(rf161KategorieAktualisieren, 2000);
});



// =====================================================
// VERSION 1.62: Kategorien in "Rezepte suchen" wie Tags
// Alte Kategorie-Kacheln werden ausgeblendet und durch stabile Chips ersetzt.
// =====================================================

(function () {
  window.rf162AktiveKategorie = "";

  function norm162(value) {
    return String(value == null ? "" : value).trim().toLowerCase().replace(/\s+/g, " ");
  }

  function alleKategorien162() {
    if (typeof KATEGORIEN !== "undefined" && Array.isArray(KATEGORIEN)) return KATEGORIEN;
    return ["Nicht zugeordnet","Kalte Vorspeisen","Suppen","Warme Vorspeisen","Salate","Saucen und Aufstriche","Hauptspeisen Fleisch","Hauptspeisen Fisch","Hauptspeisen vegetarisch","Beilagen","Dessert","Kuchen und Torten","Bäckerei","Kekse","Brot","Teige","Eis","Getränke","Haltbar machen","Gewürze","Marmeladen und Saucen","Sirup","Pralinen","Sonstiges","Airfryer"];
  }

  function escape162(value) {
    return String(value == null ? "" : value)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
  ...
  function renderKategorieChips162() {
    const oldContainer = document.getElementById("suchKategorieKacheln");
    if (!oldContainer) return;

    oldContainer.style.display = "none";

    let chipContainer = document.getElementById("rf162KategorieChips");
    if (!chipContainer) {
      chipContainer = document.createElement("div");
      chipContainer.id = "rf162KategorieChips";
      chipContainer.className = "rf162-kategorie-chips";
      oldContainer.insertAdjacentElement("afterend", chipContainer);
    }

    const kategorien = ["Alle", ...alleKategorien162()];

    chipContainer.innerHTML = kategorien.map(kat => {
      const wert = kat === "Alle" ? "" : kat;
      const aktiv = wert === "" ? !window.rf162AktiveKategorie : norm162(window.rf162AktiveKategorie) === norm162(wert);

      return `<button type="button" class="rf162-kategorie-chip ${aktiv ? "aktiv" : ""}" data-kategorie="${escape162(wert)}">${escape162(kat)}</button>`;
    }).join("");

    chipContainer.querySelectorAll(".rf162-kategorie-chip").forEach(button => {
      button.onclick = function (event) {
        if (event) {
          event.preventDefault();
          event.stopPropagation();
        ...
        const wert = button.dataset.kategorie || "";
        window.rf162AktiveKategorie = norm162(window.rf162AktiveKategorie) === norm162(wert) ? "" : wert;

        renderKategorieChips162();
        suche162();
        return false;
      };
    });
  ...
  function namePasst162(rezept) {
    const el = document.getElementById("suchNameInput");
    const q = el ? norm162(el.value) : "";
    if (!q) return true;
    return norm162(rezept.name).includes(q);
  }

  function zutatenPassen162(rezept) {
    const el = document.getElementById("suchZutatenInput");
    const q = el ? norm162(el.value) : "";
    if (!q) return true;
  ...
  function tagsPassen162(rezept) {
    const select = document.getElementById("suchTagInput");
    if (!select) return true;

    const ausgewaehlt = Array.from(select.selectedOptions || [])
      .map(o => norm162(o.value))
      .filter(Boolean);

    if (!ausgewaehlt.length) return true;

    const tags = (rezept.tags || []).map(norm162);
    return ausgewaehlt.every(tag => tags.includes(tag));
  }

  function quellePasst162(rezept) {
    const el = document.getElementById("suchQuelleInput");
    const q = el ? String(el.value || "").trim() : "";
    if (!q) return true;
    return String(rezept.quelle || "Nicht zugeordnet").trim() === q;
  }

  function statusPasst162(rezept) {
    const el = document.getElementById("suchAusprobiertInput");
    if (!el || el.value === "") return true;
    return el.value === "true" ? !!rezept.ausprobiert : !rezept.ausprobiert;
  }

  function kategoriePasst162(rezept) {
    if (!window.rf162AktiveKategorie) return true;
    return norm162(rezept.kategorie || "Nicht zugeordnet") === norm162(window.rf162AktiveKategorie);
  }

  function sortieren162(liste) {
    const el = document.getElementById("suchSortierungInput");
    const sort = el ? el.value : "name";
    const out = [...liste];

    if (sort === "bewertung") out.sort((a, b) => Number(b.bewertung || 0) - Number(a.bewertung || 0));
    else if (sort === "kategorie") out.sort((a, b) => String(a.kategorie || "").localeCompare(String(b.kategorie || "")));
  ...
  function suche162() {
    const basis = (Array.isArray(rezepte) ? rezepte : []).map((rezept, index) => ({ ...rezept, index }));

    const ergebnisse = sortieren162(basis.filter(rezept =>
      namePasst162(rezept) &&
      zutatenPassen162(rezept) &&
      tagsPassen162(rezept) &&
      quellePasst162(rezept) &&
      statusPasst162(rezept) &&
      kategoriePasst162(rezept)
    ));
  ...
  window.rf162KategorieSuche = suche162;
  ...
  const altReset162 = typeof rezeptSucheZuruecksetzen === "function" ? rezeptSucheZuruecksetzen : null;
  window.rezeptSucheZuruecksetzen = function () {
    window.rf162AktiveKategorie = "";
    if (altReset162) {
      try { altReset162(); } catch (e) {}
    }
    renderKategorieChips162();
  };
  rezeptSucheZuruecksetzen = window.rezeptSucheZuruecksetzen;

  const altToggle162 = typeof rezeptSucheToggle === "function" ? rezeptSucheToggle : null;
  window.rezeptSucheToggle = function () {
    const result = altToggle162 ? altToggle162() : undefined;
    setTimeout(renderKategorieChips162, 0);
    setTimeout(renderKategorieChips162, 300);
    return result;
  };
  rezeptSucheToggle = window.rezeptSucheToggle;

  window.addEventListener("load", function () {
    renderKategorieChips162();
    setTimeout(renderKategorieChips162, 300);
    setTimeout(renderKategorieChips162, 1000);
  });
})();
...
// =====================================================
// VERSION 1.63: Rezept löschen stabil
// Lokales Löschen wird nicht mehr durch Tags/Quelle/Cloud blockiert.
// =====================================================

function rf163SicherSpeichernNachLoeschen() {
  localStorage.setItem("rezepte", JSON.stringify(rezepte));

  try {
    if (typeof dashboardAktualisieren === "function") dashboardAktualisieren();
  } catch (e) {
    console.warn("Dashboard nach Löschen konnte nicht aktualisiert werden:", e);
  }

  try {
    if (typeof tagsListeAktualisieren === "function") tagsListeAktualisieren();
  } catch (e) {
    console.warn("Tags nach Löschen konnten nicht aktualisiert werden:", e);
  }

  try {
    if (typeof suchTagsDropdownAktualisieren === "function") suchTagsDropdownAktualisieren();
  } catch (e) {
    console.warn("Such-Tags nach Löschen konnten nicht aktualisiert werden:", e);
  }

  try {
    if (typeof quellenListeAktualisieren === "function") quellenListeAktualisieren();
  } catch (e) {
    console.warn("Quellen nach Löschen konnten nicht aktualisiert werden:", e);
  }

  try {
    if (typeof quellenDropdownAktualisieren === "function") quellenDropdownAktualisieren();
  } catch (e) {
    console.warn("Quellen-Dropdown nach Löschen konnte nicht aktualisiert werden:", e);
  }
}

function rf163AnsichtNachLoeschenAktualisieren() {
  try {
    if (typeof rezeptSucheAusfuehren === "function") {
      rezeptSucheAusfuehren();
      return;
    ...
    console.warn("Suche nach Löschen konnte nicht aktualisiert werden:", e);
  }
  ...
  try {
    ...
    console.warn("Filter nach Löschen konnte nicht aktualisiert werden:", e);
  }
  ...
  try {
    ...
    console.warn("Ergebnisliste nach Löschen konnte nicht aktualisiert werden:", e);
  }
}

function rf163CloudNachLoeschen(rezept) {
  try {
  ...
  try {
    if (typeof cloudSpeichernAlle === "function") {
      cloudSpeichernAlle();
    }
  } catch (e) {
    console.warn("Cloud-Gesamtsync nach Löschen fehlgeschlagen:", e);
  }
}

function rf163RezeptLoeschen(index) {
  try {
    if (!Array.isArray(rezepte)) rezepte = [];

    const i = Number(index);

    if (!Number.isInteger(i) || i < 0 || i >= rezepte.length) {
      alert("Rezept konnte nicht gefunden werden.");
      return false;
    }

    const rezept = rezepte[i];
    const rezeptName = rezept && rezept.name ? rezept.name : "dieses Rezept";

    if (!confirm(`Möchtest du "${rezeptName}" wirklich löschen?`)) {
      return false;
    }

    zuletztGeloeschtesRezept = rezept;
    zuletztGeloeschterIndex = i;

    rezepte.splice(i, 1);

    if (bearbeitungsIndex === i) {
      bearbeitungsIndex = null;
    } else if (bearbeitungsIndex !== null && bearbeitungsIndex > i) {
      bearbeitungsIndex -= 1;
    }

    // Wichtig: zuerst dauerhaft lokal speichern.
    rf163SicherSpeichernNachLoeschen();

    // Danach Cloud/Nebenfunktionen, aber ohne den lokalen Löschvorgang zu gefährden.
    rf163CloudNachLoeschen(rezept);

    if (typeof meldungAnzeigen === "function") {
      meldungAnzeigen(`Rezept "${rezeptName}" gelöscht.`);
    }

    rf163AnsichtNachLoeschenAktualisieren();

    return true;
  } catch (fehler) {
    console.error("Rezept löschen Fehler:", fehler);
    alert("Rezept konnte nicht gelöscht werden: " + (fehler.message || "unbekannter Fehler"));
    return false;
  }
}

// Alle alten Button-Aufrufe auf die stabile Löschfunktion legen.
function rezeptLoeschen(index) {
  return rf163RezeptLoeschen(index);
}

window.rf163RezeptLoeschen = rf163RezeptLoeschen;
window.rezeptLoeschen = rf163RezeptLoeschen;



// =====================================================
// VERSION 1.64: Löschen endgültig dauerhaft
// - Löschen per Index oder ID
// - Tombstone-Liste verhindert Wiederherstellung aus Cloud
// - Cloud-Download wird nachträglich gefiltert
// =====================================================

function rf164GelöschteIdsLaden() {
  try {
    return JSON.parse(localStorage.getItem("geloeschteRezeptIds") || "[]");
  } catch (e) {
    return [];
  }
}

function rf164GelöschteIdsSpeichern(ids) {
  try {
    localStorage.setItem("geloeschteRezeptIds", JSON.stringify([...new Set(ids.filter(Boolean).map(String))]));
  } catch (e) {
    console.warn("Gelöschte IDs konnten nicht gespeichert werden:", e);
  }
}

function rf164AlsGeloeschtMerken(rezept) {
  if (!rezept) return;
  const ids = rf164GelöschteIdsLaden();

  if (rezept.id) ids.push(String(rezept.id));
  if (rezept.cloud_id) ids.push(String(rezept.cloud_id));

  // Falls alte Test-/Importrezepte keine ID hatten, bekommen sie trotzdem eine stabile Löschmarke.
  const nameKey = "name:" + String(rezept.name || "").trim().toLowerCase() + "|" + String(rezept.quelle || "").trim().toLowerCase();
  if (rezept.name) ids.push(nameKey);

  rf164GelöschteIdsSpeichern(ids);
}

function rf164IstGeloescht(rezept) {
  if (!rezept) return false;
  const ids = new Set(rf164GelöschteIdsLaden().map(String));

  if (rezept.id && ids.has(String(rezept.id))) return true;
  if (rezept.cloud_id && ids.has(String(rezept.cloud_id))) return true;

  const nameKey = "name:" + String(rezept.name || "").trim().toLowerCase() + "|" + String(rezept.quelle || "").trim().toLowerCase();
  if (rezept.name && ids.has(nameKey)) return true;

  return false;
}

function rf164GeloeschteAusListeFiltern() {
  if (!Array.isArray(rezepte)) rezepte = [];
  const vorher = rezepte.length;
  rezepte = rezepte.filter(r => !rf164IstGeloescht(r));

  if (rezepte.length !== vorher) {
    try {
      localStorage.setItem("rezepte", JSON.stringify(rezepte));
    } catch (e) {
      console.warn("Gefilterte Rezeptliste konnte nicht gespeichert werden:", e);
    }
  }
}

function rf164IndexFinden(indexOderId) {
  if (!Array.isArray(rezepte)) rezepte = [];
  ...
  return rezepte.findIndex(r => String(r.id || "") === id || String(r.cloud_id || "") === id);
}

function rf164NachLoeschenSpeichern() {
  localStorage.setItem("rezepte", JSON.stringify(rezepte));
...
function rf164CloudDelete(rezept) {
  try {
    if (rezept && rezept.id && typeof supabaseClient !== "undefined" && supabaseClient) {
      supabaseClient.from("rezepte").delete().eq("id", String(rezept.id));
    }
...
function rf164AnsichtAktualisieren() {
  try {
    if (typeof rezeptSucheAusfuehren === "function") {
      rezeptSucheAusfuehren();
      return;
...
function rf164RezeptLoeschen(indexOderId) {
  try {
    rf164GeloeschteAusListeFiltern();

    const i = rf164IndexFinden(indexOderId);
    ...
    rf164AlsGeloeschtMerken(rezept);
    ...
    if (typeof bearbeitungsIndex !== "undefined") {
      if (bearbeitungsIndex === i) bearbeitungsIndex = null;
      else if (bearbeitungsIndex !== null && bearbeitungsIndex > i) bearbeitungsIndex -= 1;
    }

    rf164NachLoeschenSpeichern();
    rf164CloudDelete(rezept);
    rf164AnsichtAktualisieren();
...
function rezeptLoeschen(indexOderId) {
  return rf164RezeptLoeschen(indexOderId);
}

window.rezeptLoeschen = rf164RezeptLoeschen;
window.rf164RezeptLoeschen = rf164RezeptLoeschen;

// Cloud-/Ladefunktionen absichern: falls gelöschte Rezepte wieder hereinkommen, sofort entfernen.
function rf164CloudFilterNachziehen() {
  try {
    rf164GeloeschteAusListeFiltern();
    rf164AnsichtAktualisieren();
  } catch (e) {
    console.warn("Cloud-Filter nach Löschen fehlgeschlagen:", e);
  }
}

window.addEventListener("load", function () {
  rf164GeloeschteAusListeFiltern();
  setTimeout(rf164CloudFilterNachziehen, 300);
  setTimeout(rf164CloudFilterNachziehen, 1500);
  setTimeout(rf164CloudFilterNachziehen, 4000);
});

const rf164OriginalCloudLaden = typeof cloudLaden === "function" ? cloudLaden : null;
if (rf164OriginalCloudLaden) {
  cloudLaden = async function () {
    const result = await rf164OriginalCloudLaden.apply(this, arguments);
    rf164GeloeschteAusListeFiltern();
    rf164AnsichtAktualisieren();
    return result;
  };
...
const rf164OriginalCloudHerunterladen = typeof cloudHerunterladen === "function" ? cloudHerunterladen : null;
if (rf164OriginalCloudHerunterladen) {
  cloudHerunterladen = async function () {
    const result = await rf164OriginalCloudHerunterladen.apply(this, arguments);
    rf164GeloeschteAusListeFiltern();
    rf164AnsichtAktualisieren();
    return result;
  };
...
// =====================================================
// VERSION 1.65: Löschen wirklich wirksam
// Gelöschte Rezepte werden in jeder Ansicht/Suche ausgeblendet,
// auch wenn sie durch Cloud-Download wieder in die Liste kommen.
// =====================================================
...
(function () {
  ...
  function rf176LocalSaveRecipe() {
    try {
      ...
      console.error("Lokales Speichern v1.76 fehlgeschlagen:", e);
      alert("Rezept konnte nicht gespeichert werden: " + (e.message || "unbekannter Fehler"));
      return false;
    }
  }

  async function rf176Client() {
    try { if (typeof cloudInit === "function") cloudInit(); } catch(e) {}
    const client = typeof supabaseClient !== "undefined" ? supabaseClient : null;
    if (!client) throw new Error("Supabase ist nicht verbunden.");
    return client;
...
(function () {
  ...
  function rf177Status(text, error) {
    try {
      if (typeof cloudStatus === "function") cloudStatus(text, !!error);
      else console.log(text);
    } catch (e) {
  ...
  window.addEventListener("load", function () {
    rf177BindButtons();
    setTimeout(rf177BindButtons, 500);
    setTimeout(rf177BindButtons, 1500);
  });
})();
...
(function () {
  ...
  function rf179LocalSaveRecipe() {
    try {
      ...
      console.error("Speichern v1.79 fehlgeschlagen:", e);
      alert("Rezept konnte nicht gespeichert werden: " + (e.message || "unbekannter Fehler"));
      return false;
    }
  }

  const oldEdit179 = typeof rezeptBearbeiten === "function" ? rezeptBearbeiten : null;
  if (oldEdit179) {
    rezeptBearbeiten = function(index) {
      rf179CleanDeletedKeys();
      rf179LoadLocal();
