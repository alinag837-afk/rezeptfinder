...
// VERSION 2.06 Auto-Update wurde in v2.07 deaktiviert.



// =====================================================
// VERSION 2.07: Stabiles Update ohne Auto-Reload-Loop
// =====================================================

(function () {
  const RF207_VERSION = "2.07";
  const RF207_BUILD = "1779793356";

  async function rf207ClearBrowserCachesOnly() {
    try {
      if ("serviceWorker" in navigator) {
        const regs = await navigator.serviceWorker.getRegistrations();
        for (const reg of regs) {
  ...
  async function rf207ManualUpdate() {
    await rf207ClearBrowserCachesOnly();

    const url = new URL(window.location.href);
    url.searchParams.set("v", RF207_VERSION + "-" + RF207_BUILD);
    url.searchParams.set("reload", Date.now().toString());

    window.location.href = url.toString();
  }

  function rf207ShowCorrectVersion() {
    const footer = document.querySelector("footer");
    if (footer) footer.textContent = "Rezeptfinder v" + RF207_VERSION + " Cloud";

    const btn = document.getElementById("rf207UpdateButton");
    if (btn) {
      btn.onclick = rf207ManualUpdate;
      btn.textContent = "Update neu laden";
    }
  }

  window.rf207ManualUpdate = rf207ManualUpdate;
  window.rf207ClearBrowserCachesOnly = rf207ClearBrowserCachesOnly;

  window.addEventListener("load", function () {
    // Kein automatischer Reload mehr. Nur Anzeige + manueller Button.
    rf207ShowCorrectVersion();
    localStorage.setItem("rezeptfinder_version", RF207_VERSION + "-" + RF207_BUILD);
  });
})();
