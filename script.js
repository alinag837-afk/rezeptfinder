...
// =====================================================
// VERSION 2.04 Backup Button fest sichtbar erzwingen
// =====================================================
...
(function () {
  ...
  function rf204EnsurePanel() {
    let panel = document.getElementById("rf204BackupPanel");
    if (!panel) {
      panel = document.createElement("section");
      panel.id = "rf204BackupPanel";
      panel.className = "box versteckt";
      panel.innerHTML = `
        <h2>Backup</h2>
        <p>Hier findest du deine Backup- und Cloud-Funktionen.</p>
        <div id="rf204BackupButtons" class="button-gruppe"></div>
      `;
  ...
  function rf204BackupToggle() {
    const panel = rf204EnsurePanel();
    const btn = document.getElementById("rf204BackupToggle");
    ...
    if (closed) {
      ...
      panel.style.display = "";
      panel.hidden = false;
      if (btn) btn.textContent = "Backup einklappen";
    } else {
      panel.classList.add("versteckt");
      panel.style.display = "none";
      panel.hidden = true;
      if (btn) btn.textContent = "Backup anzeigen";
    }

    rf204EnsureBackupButtons();
    return false;
  }

  function rf204EnsureBackupButton() {
    const group = rf204FindStartGroup();
    if (!group) return;

    let btn = document.getElementById("rf204BackupToggle");

    if (!btn) {
      btn = document.createElement("button");
      btn.id = "rf204BackupToggle";
      btn.type = "button";
      btn.textContent = "Backup anzeigen";
    }

    btn.onclick = rf204BackupToggle;
    btn.hidden = false;
    btn.style.display = "inline-flex";

    // Fest nach "Rezepte prüfen" setzen, damit das Raster gleich bleibt.
    const pruefen = Array.from(group.querySelectorAll("button")).find(b =>
      (b.textContent || "").trim().toLowerCase() === "rezepte prüfen"
    );

    if (pruefen && pruefen.nextSibling !== btn) {
      pruefen.insertAdjacentElement("afterend", btn);
    } else if (!group.contains(btn)) {
      group.appendChild(btn);
    }
  }

  function rf204EnsureBackupButtons() {
    const panel = rf204EnsurePanel();
    const box = document.getElementById("rf204BackupButtons");
    if (!box) return;

    const needed = [
      ["Jetzt in Cloud speichern", "cloudSpeichernAlle"],
      ["Aus Cloud laden", "cloudLaden"],
      ["Cloud-Backups anzeigen", "cloudBackupsAnzeigen"],
      ["Manuelles Backup herunterladen", "backupHerunterladen"]
    ];

    needed.forEach(([label, fn]) => {
      let existing = Array.from(box.querySelectorAll("button")).find(b =>
      ...
      if (!existing) {
        ...
        existing.onclick = function () {
          ...
          if (fn === "cloudBackupsAnzeigen" && typeof cloudBackupAnzeigen === "function") return cloudBackupAnzeigen();
          if (fn === "backupHerunterladen" && typeof backupExportieren === "function") return backupExportieren();
          alert("Diese Backup-Funktion ist aktuell nicht verfügbar.");
          return false;
        };
  ...
  // Alte Skripte dürfen den Button nicht entfernen oder wegschieben.
  function rf204KeepAlive() {
    rf204EnsureBackupButton();
    rf204EnsurePanel();
  ...
  window.rf204BackupToggle = rf204BackupToggle;
  window.rf204KeepAlive = rf204KeepAlive;

  window.addEventListener("load", function () {
    rf204KeepAlive();
    setTimeout(rf204KeepAlive, 100);
    setTimeout(rf204KeepAlive, 500);
    setTimeout(rf204KeepAlive, 1200);
    setTimeout(rf204KeepAlive, 2500);
  });
})();


// VERSION 2.05 Backup Button ohne Blinken
window.addEventListener("load", function () {
  setTimeout(function () {
    const btn = document.getElementById("rf204BackupToggle");
    if (!btn) return;
    ...
    // Nicht mehr ständig neu verschieben
    const keepAlive = window.rf204KeepAlive;
    if (keepAlive) {
      window.rf204KeepAlive = function(){};
    }

    const group = btn.parentElement;
    if (group && !group.contains(btn)) {
      group.appendChild(btn);
    }
  }, 50);
});
...
// =====================================================
// VERSION 2.06: Auto-Update und Cache-Fix
// =====================================================

(function () {
  const RF206_VERSION = "2.06";
  const RF206_BUILD = "1779793154";
  const RF206_KEY = "rezeptfinder_loaded_version";

  async function rf206ClearCaches() {
    try {
      if ("serviceWorker" in navigator) {
        const regs = await navigator.serviceWorker.getRegistrations();
        for (const reg of regs) {
  ...
  function rf206UrlWithVersion() {
    const url = new URL(window.location.href);
    url.searchParams.set("v", RF206_VERSION + "-" + RF206_BUILD);
    url.searchParams.set("t", Date.now().toString());
    return url.toString();
  }

  async function rf206ForceUpdate() {
    try {
      localStorage.setItem(RF206_KEY, RF206_VERSION + "-" + RF206_BUILD);
    } catch (e) {}

    await rf206ClearCaches();

    // Cache-busting reload
    window.location.replace(rf206UrlWithVersion());
  }

  async function rf206AutoUpdateCheck() {
    let loaded = "";
    try {
      loaded = localStorage.getItem(RF206_KEY) || "";
    } catch (e) {}

    const current = RF206_VERSION + "-" + RF206_BUILD;

    // Beim ersten Laden der neuen Version: Caches löschen, aber nicht endlos neu laden.
    if (loaded !== current) {
      try {
        localStorage.setItem(RF206_KEY, current);
      } catch (e) {}

      await rf206ClearCaches();

      // Nur reloaden, wenn URL noch keinen aktuellen v-Parameter hat.
      const url = new URL(window.location.href);
  ...
  function rf206ShowVersion() {
    const footer = document.querySelector("footer");
    if (footer) footer.textContent = "Rezeptfinder v" + RF206_VERSION + " Cloud";
  }

  window.rf206ForceUpdate = rf206ForceUpdate;
  window.rf206ClearCaches = rf206ClearCaches;

  window.addEventListener("load", function () {
    rf206ShowVersion();
    rf206AutoUpdateCheck();

    const btn = document.getElementById("rf206UpdateButton");
    if (btn) {
      btn.onclick = rf206ForceUpdate;
      btn.textContent = "Update neu laden";
    }
  });
})();
